...
if onPi and OBDEnabled and developermode == 0:
    try:
        ...
        OBDEnabled = 0
        print(
            "Did you install obd? - https://python-obd.readthedocs.io/en/latest/#installation"
        )
else:
    OBDEnabled = 0
...
if externalshutdown:
    ExternalShutdownCheckThread = threading.Thread(
        name="check_externalshutdown_thread", target=CheckExternalShutdown
    )
...
class vehicle:
    ...
    def findgear(self, RPM, Speed):
        if Speed == 0 or RPM < 1500:
            ...
            ratiocalcd = (
                0.00595 * RPM * vehicle.gear.tirerad / (vehicle.gear.reduction * Speed)
            )
...
class OBD:
    ...
    def OBD_setup_thread(self):
        ...
        try:
            ...
            try:
                MainApp.show_warning(
                    self, "Error setting OBD vars", "OBD is now disabled"
                )
            except:
    ...
    def OBD_update_thread(self):
        ...
        while OBDEnabled and OBD.Connected:
            if OBD.enable.Speed:
                try:
                    response_SPEED = OBD.connection.query(
                        OBD.cmd_Speed
                    )  # send the command, and parse the response
                    if (
                        str(response_SPEED.value.magnitude) != "None"
                    ):  # only proceed if string value is not None
                        OBD.Speed = int(
                            response_SPEED.value.magnitude * 0.6213711922
                        )  # Set int value
            ...
            if OBD.enable.CoolantTemp:
                try:
                    ...
                    if str(response_CoolantTemp.value.magnitude) != "None":
                        OBD.CoolantTemp = int(
                            response_CoolantTemp.value.magnitude * 9.0 / 5.0 + 32.0
                        )
                except:
            ...
            if OBD.enable.IntakeTemp:
                try:
                    ...
                    if str(response_IntakeTemp.value.magnitude) != "None":
                        OBD.IntakeTemp = int(
                            response_IntakeTemp.value.magnitude * 9.0 / 5.0 + 32.0
                        )
                except:
            ...
            if OBD.enable.IntakePressure:
                try:
                    response_IntakePressure = OBD.connection.query(
                        OBD.cmd_IntakePressure
                    )
                    if str(response_IntakePressure.value.magnitude) != "None":
                        OBD.IntakePressure = int(
                            response_IntakePressure.value.magnitude
                        )  # kPa
                except:
            ...
            if OBD.enable.WarmUpsSinceDTC:
                try:
                    response_WarmUpsSinceDTC = OBD.connection.query(
                        OBD.cmd_WarmUpsSinceDTC
                    )
                    if str(response_WarmUpsSinceDTC.value.magnitude) != "None":
                        OBD.WarmUpsSinceDTC = int(
                            response_WarmUpsSinceDTC.value.magnitude
                        )
                except:
            ...
            if OBD.enable.DistanceSinceDTC:
                try:
                    response_DistanceSinceDTC = OBD.connection.query(
                        OBD.cmd_DistanceSinceDTC
                    )
                    if str(response_DistanceSinceDTC.value.magnitude) != "None":
                        OBD.DistanceSinceDTC = int(
                            response_DistanceSinceDTC.value.magnitude * 0.6213711922
                        )
                except:
            ...
            if OBD.enable.CatTemp:
                try:
                    ...
                    if str(response_CatTemp.value.magnitude) != "None":
                        OBD.CatTemp = int(
                            response_CatTemp.value.magnitude * 9.0 / 5.0 + 32.0
                        )
                except:
                    print("Could not get OBD Response - Cat Temp")
            if OBD.enable.EGT1:
                try:
                    response_EGT1 = OBD.connection.query(OBD.cmd_EGT1)
                    if str(response_EGT1.value.magnitude) != "None":
                        OBD.EGT1 = int(response_EGT1.value.magnitude * 9.0 / 5.0 + 32.0)
                except:
                    print("Could not get OBD Response - EGT1 Temp")
            if OBD.enable.EGT2:
                try:
                    response_EGT2 = OBD.connection.query(OBD.cmd_EGT2)
                    if str(response_EGT2.value.magnitude) != "None":
                        OBD.EGT2 = int(response_EGT2.value.magnitude * 9.0 / 5.0 + 32.0)
                except:
                    print("Could not get OBD Response - EGT2 Temp")
            if OBD.enable.CACT1:
                try:
                    response_CACT1 = OBD.connection.query(OBD.cmd_CACT1)
                    if str(response_CACT1.value.magnitude) != "None":
                        OBD.CACT1 = int(
                            response_CACT1.value.magnitude * 9.0 / 5.0 + 32.0
                        )
                except:
                    print("Could not get OBD Response - CACT1 Temp")
            if OBD.enable.TurboBoost:
                try:
                    response_TurboBoost = OBD.connection.query(OBD.cmd_TurboBoost)
                    if str(response_TurboBoost.value.magnitude) != "None":
                        OBD.TurboBoost = int(
                            response_TurboBoost.value.magnitude * 0.1450377377
                        )
                except:
                    print("Could not get OBD Response - Turbo Boost PSI")

    def start_setup_thread(self):
        OBDSetupThread = threading.Thread(
            name="obd_setup_thread", target=self.OBD_setup_thread
        )
        OBDSetupThread.start()

    def start_update_thread(self):
        OBDUpdateThread = threading.Thread(
            name="obd_update_thread", target=self.OBD_update_thread
        )
...
if developermode == 0:
    if autobrightness == 1:
        ...
        if (
            currenthour < 7 or currenthour >= 20
        ):  # earlier than 7am and later than 6pm -> dim screen on start
...
class MainApp(App):
    def build(self):
        Clock.schedule_interval(self.updatevariables, 0.1)
        Clock.schedule_interval(self.updateOBDdata, 0.01)
    ...
    EGT1 = NumericProperty(0)
    EGT2 = NumericProperty(0)
    CACT1 = NumericProperty(0)
    TurboBoost = NumericProperty(0)
    ...
    def updateOBDdata(self, *args):
        ...
        if OBD.Connected == 0 and developermode:
            ...
            # no dev for EGT1+2 + CACT1 + TurboBoost implemented

        # S2K Bar Image Selection
        if (
            OBD.enable.CoolantTemp
            and 0
            <= int(round(self.CoolantTemp / OBD.gauge.persegment.CoolantTemp))
            <= 32
        ):
            self.CoolantTemp_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.CoolantTemp / OBD.gauge.persegment.CoolantTemp))))
                + ".png"
            )
        if (
            OBD.enable.IntakeTemp
            and 0 <= int(round(self.IntakeTemp / OBD.gauge.persegment.IntakeTemp)) <= 32
        ):
            self.IntakeTemp_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.IntakeTemp / OBD.gauge.persegment.IntakeTemp))))
                + ".png"
            )
        if (
            OBD.enable.Voltage
            and 0 <= int(round(self.Voltage / OBD.gauge.persegment.Voltage)) <= 32
        ):
            self.Voltage_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.Voltage / OBD.gauge.persegment.Voltage))))
                + ".png"
            )
        if (
            OBD.enable.STFT
            and -16 <= int(round(self.STFT / OBD.gauge.persegment.STFT)) <= 16
        ):
            self.STFT_Image = str(
                "data/gauges/split/s2k_"
                + (str(int(round(self.STFT / OBD.gauge.persegment.STFT))))
                + ".png"
            )
        if (
            OBD.enable.LTFT
            and -16 <= int(round(self.LTFT / OBD.gauge.persegment.LTFT)) <= 16
        ):
            self.LTFT_Image = str(
                "data/gauges/split/s2k_"
                + (str(int(round(self.LTFT / OBD.gauge.persegment.LTFT))))
                + ".png"
            )
        if (
            OBD.enable.ThrottlePos
            and 0
            <= int(round(self.ThrottlePos / OBD.gauge.persegment.ThrottlePos))
            <= 32
        ):
            self.ThrottlePos_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.ThrottlePos / OBD.gauge.persegment.ThrottlePos))))
                + ".png"
            )
        if (
            OBD.enable.Load
            and 0 <= int(round(self.Load / OBD.gauge.persegment.Load)) <= 32
        ):
            self.Load_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.Load / OBD.gauge.persegment.Load))))
                + ".png"
            )
        if (
            OBD.enable.TimingAdv
            and 0 <= int(round(self.TimingAdv / OBD.gauge.persegment.TimingAdv)) <= 32
        ):
            self.TimingAdv_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.TimingAdv / OBD.gauge.persegment.TimingAdv))))
                + ".png"
            )
        if (
            OBD.enable.EGT1
        ...
        if (
            OBD.enable.TurboBoost
            and 0 <= int(round(self.TurboBoost / OBD.gauge.persegment.TurboBoost)) <= 32
        ):
            self.TurboBoost_Image = str(
                "data/gauges/normal/s2k_"
                + (str(int(round(self.TurboBoost / OBD.gauge.persegment.TurboBoost))))
                + ".png"
            )
    ...
    def OBDEnabler(obj, PID, status):
        setattr(
            OBD.enable, PID, status
        )  # sets OBD enable class based on PID (text) and Status (int) input
